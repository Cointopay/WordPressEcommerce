...
function c2pCreateInvoice($orderId, $price, $posData, $options = array()) {	
	...
	$result = c2pCurl('https://cointopay.com/MerchantAPI?Checkout=true&SecurityCode=' .  $options['cointopay_SecurityCode'] . '&MerchantID='.$options['cointopay_MerchantID'].'&Amount='.$price.'&AltCoinID='.$options['defaultCoin'].'&output=json&CustomerReferenceNr='.$options['orderID'].'&inputCurrency=USD&returnurl='.rawurlencode(esc_url(site_url().'/?wc-api=Cointopay')).'&transactionconfirmurl='.site_url('/?wc-api=Cointopay') .'&transactionfailurl='.rawurlencode(esc_url('www.cointopay.com')), $options['apiKey'], $post);
